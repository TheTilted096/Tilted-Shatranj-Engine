...
int fullMoveGen(int ply, bool capsOnly){
    ...
    for (int ll : leapers){
        ...
        while (tempBoard){
            ...
            if (capsOnly){ moveSet &= pieces[7 * (!toMove) + 6];}
            q = -1;
            while (moveSet){
                p = __builtin_ctzll(moveSet);
                q += (p + 1);
...
void makeMove(uint32_t move, bool forward, bool ev){
    ...
    if (ev){
        int psb = mps[typeEnded][endsquare ^ (!color * 56)] 
            - mps[typeMoved][startsquare ^ (!color * 56)];
...
bool isChecked(){ //checks if the opposing side is left in check; called after makeMove
    ...
    phantomSet |= ((uint64_t) hlt[kingSquare & 7][((((RANK0 << ((kingSquare & 56)))
         & (pieces[6] | pieces[13])) >> (kingSquare & 56)) >> 1) & 63])
